<x-app-layout>
    <div class="max-w-4xl mx-auto mt-10 bg-white p-6 rounded-lg shadow">
        ...
        <form id="thumbnailForm" action="{{ route('thumbnails.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Background Image (Rasio 16:9)</label>
                ...
                <div id="backgroundResult" class="mt-4 hidden">
                    <p class="text-green-600 mb-2">✓ Background image sudah di-crop</p>
                    <img id="backgroundCropped" style="max-width: 400px; border: 2px solid #10b981;">
                </div>
                <input type="hidden" name="background_image_cropped" id="backgroundCroppedData" required>
            </div>

            <!-- Gambar Tambahan (4:3) -->
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Gambar Tambahan (Rasio 4:3) - Opsional</label>
    ...
    <script>
        ...
        document.getElementById('thumbnailForm').addEventListener('submit', function(e) {
            const bgData = document.getElementById('backgroundCroppedData').value;
            if (!bgData) {
                e.preventDefault();
                alert('Harap crop background image terlebih dahulu!');
                return false;
            }
        });
    </script>
